...
public class Video {
	...
	public Video(VideoName videoName, File videoFile, DurationMinutes durationMinutes, InputStream inputStream,
			MinuteFrameCut minuteFrameCut, Metadata metadata) {
	 ...
	 *
	 * <p>
	 * Antes de atribuir o valor, valida se o tempo configurado para corte de frames
	 * ({@link MinuteFrameCut}) é menor que a duração total do vídeo. Essa validação
	 * garante que o processo de captura de imagens não tente acessar instantes
	 * inexistentes no vídeo.
	 * </p>
	 *
	 * @param durationMinutes
	 *            objeto de valor que representa a duração total do vídeo em minutos
	 * @throws VideoException
	 *             caso o tempo de corte para captura de imagens seja maior ou igual
	 *             à duração total do vídeo
	 */
	public void setDurationMinutes(DurationMinutes durationMinutes) {
		...
		if (this.minuteFrameCut.value() >= durationMinutes.value())
			throw new VideoException(
					"O tempo de corte para captura de imagens deve ser menor que a duração total do vídeo");
