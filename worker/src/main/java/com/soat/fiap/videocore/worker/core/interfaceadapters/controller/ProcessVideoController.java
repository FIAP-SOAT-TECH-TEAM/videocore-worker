...
/**
 * Controller responsável por orquestrar o processamento de vídeos a partir de
 * eventos de criação de blob.
 */
...
public class ProcessVideoController {
	...
	/**
	 * Orquestra o fluxo de processamento de um vídeo recém-criado no blob storage.
	 * Capturando suas imagens e enviando para um destino
	 *
	 * @param payload
	 *            evento CloudEvent contendo dados do blob
	 */
	...
	public void processVideo(BlobCreatedCloudEventSchemaPayload payload) {
		...
		try {
			...
			publishVideoStatusProcessUpdateUseCase.publishVideoStatusProcessUpdate(video,
					videoProcessMetadata.getCurrentPercent(), videoProcessMetadata.getImageMinute(), true);
			throw e;
		} finally {
