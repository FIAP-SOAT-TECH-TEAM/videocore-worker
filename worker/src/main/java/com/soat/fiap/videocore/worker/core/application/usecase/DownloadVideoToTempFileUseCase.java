...
/**
 * Caso de uso responsável por realizar o download de um vídeo a partir de seu {@link java.io.InputStream}
 * e persistí-lo temporariamente no sistema de arquivos local.
 * <p>
 * O arquivo gerado é criado no diretório temporário padrão do sistema operacional e deve ser removido
 * explicitamente pelo chamador após o uso, evitando vazamento de recursos em disco.
 * <p>
 * Este caso de uso é indicado quando a API consumidora exige um {@link File} ou um recurso seekable
 * (ex.: bibliotecas que dependem de acesso aleatório ao conteúdo do vídeo).
 */
@Component
...
public class DownloadVideoToTempFileUseCase {
    ...
    /**
     * Efetua o download do vídeo representado pelo objeto de domínio {@link Video}. Então, popula as propriedades correspondentes do objeto de domínio
     *
    ...
    public void downloadVideoToTempFile(Video video) {
        ...
        try (var inputStream = video.getInputStream()) {
            var tempFileName = String.format(
                    "video-%s-%s-%s",
                    video.getMetadata().requestId(),
                    video.getMetadata().userId(),
                    video.getVideoName()
            );
