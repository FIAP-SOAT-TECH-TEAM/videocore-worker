...
class ProcessVideoControllerTest {
	...
	void shouldProcessVideoSuccessfully() {
		...
		PublishVideoStatusProcessUpdateUseCase publishVideoStatusProcessUpdateUseCase = mock(
				PublishVideoStatusProcessUpdateUseCase.class);

		ProcessVideoController controller = new ProcessVideoController(getVideoUseCase, getZipOutputStreamUseCase,
				downloadVideoToTempFileUseCase, getVideoDurationMinutesUseCase, processVideoUseCase,
				deleteVideoFileUseCase, publishVideoStatusProcessUpdateUseCase);
		...
		BlobCreatedCloudEventSchemaPayload.DataPayload data = mock(
		...
		verify(processVideoUseCase).processVideo(eq(video), eq(zipOutputStream), any(VideoProcessMetadataInput.class));
	...
	void shouldPublishErrorEventAndDeleteFileWhenExceptionOccurs() {
		...
		when(data.url()).thenReturn("video-url");
		when(getVideoUseCase.getVideo(any())).thenReturn(video);
		doThrow(new RuntimeException("erro")).when(downloadVideoToTempFileUseCase).downloadVideoToTempFile(video);

		// Act
		RuntimeException exception = assertThrows(RuntimeException.class, () -> controller.processVideo(payload));

		// Assert
		assertEquals("erro", exception.getMessage());
		verify(publishVideoStatusProcessUpdateUseCase).publishVideoStatusProcessUpdate(eq(video), any(), anyLong(),
				eq(true));
		verify(deleteVideoFileUseCase).deleteVideoFile(video);
	}
	...
	void shouldPublishErrorEventWhenExceptionOccursBeforeVideoIsLoaded() {
		...
		// Assert
		assertEquals("erro", exception.getMessage());
		verify(publishVideoStatusProcessUpdateUseCase).publishVideoStatusProcessUpdate(isNull(), any(), anyLong(),
				eq(true));
